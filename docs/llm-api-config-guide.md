...
> **维护规则**：模型名称、端点、套餐名称和废弃日期都属于高变动信息。更新本文件时必须重新核验官方文档，并在相关条目中保留核验月份；不要仅凭历史记录或第三方转述更新模型清单。

## 核验流程

1. 优先查看厂商官方 API 文档、模型列表、定价页或 changelog。
2. 记录核验月份；如果厂商没有公开稳定模型清单，写“以控制台/官方模型列表为准”。
3. 对即将废弃、迁移或套餐限定端点，必须写明具体日期和适用范围。
4. 不确定的信息不要写成默认推荐项；放入备注或暂不收录。
