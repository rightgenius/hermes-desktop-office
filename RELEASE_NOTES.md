...
- 新增结构化错误分类，自动识别 10+ 种 API 错误类型：
  - 🔑 **认证失败** (401)：API Key 无效或过期
  - ⏳ **限流** (429)：请求过于频繁
  - 🌐 **网络/超时**：连接超时、网络错误、代理问题
  - ⚠️ **服务不可用** (503)：服务器过载
  - 💰 **余额不足** (402)：账户配额耗尽
  - 📝 **对话过长**：上下文超出模型限制
  - 🤖 **模型不存在**：模型名称错误
...
- 支持自动重试的错误类型：限流、超时、网络错误、服务不可用、网关错误
- 指数退避 + 抖动：2s → 4s → 8s，最大延迟 30 秒
- 最多自动重试 3 次
- 重试过程中显示倒计时，可手动取消
- 用户发送新消息时自动取消之前的重试
...
- **Markdown 渲染优化**：streaming 时使用增量 DOM 更新，复杂 Markdown 触发 debounce 渲染（100ms），避免每次 chunk 全量重解析
- **消息数量限制**：每个 session 最多保留 100 条消息，自动截断旧消息
- **localStorage 保护**：总大小限制 4MB，超出时自动清理最旧 session
- **历史消息懒加载**：打开 session 时只加载最近 30 条消息，点击"加载更多"按需获取历史

### 界面体验

- 错误消息使用彩色卡片展示，与正常回复明显区分
- streaming 消息添加闪烁光标动画
- 重试通知使用黄色渐变背景，与普通消息区分
- 加载更多按钮使用虚线边框和悬停动效

## 测试

- 新增 Python 单元测试 (26 个用例)：覆盖错误分类的所有场景
- 新增 Node.js 单元测试 (42 个用例)：覆盖重试逻辑、消息限制、Markdown 检测等
- 新增 Playwright E2E 测试 (16 个用例)：验证错误 UI 组件和配置
- 所有测试通过

## 文档

- 新增 `docs/performance-and-error-handling-spec.md`：详细设计文档，包含实现方案、测试计划、风险评估
